...
function [stress,constitutive_tensor] = Material_2(stress,dstrain,properties)
...
% elastic predictor
	...
	for i = 1:6
		for j = 1:6
			stress(i) = stress(i) + constitutive_tensor(i,j) * dstrain(j);
		end
	end
...
% trial stress
	for i = 1:6
		trial_stress(i) = stress(i) - alpha(i);
	end
	hydrostatic_stress = (1/3) * (stress(1) + stress(2) + stress(3));
	for i = 1:3
...
% plastic corrector
	...
	if abs(effective_trial_stress) > sqrt(2/3) * fkappa(properties(3), properties(4), properties(5), history_variables(7))
		...
		for i = 1:6
			...
			stress(i) = sqrt(2/3) * fkappa(properties(3),properties(4),properties(5),history_variables(7)) * unit_trial_stress(i) + alpha(i);
			history_variables(i) = alpha(i);
		end
		for i = 1:3
			stress(i) = stress(i) + hydrostatic_stress;
		end
